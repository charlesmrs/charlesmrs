<div align="center">

# Olá, sou Charles Moraes Rodrigues 👋

### Engenheiro de Automação Industrial → Machine Learning & IA Industrial

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/charles-moraes-rodrigues-06250967)
[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:charlesmrs@gmail.com)
[![Localização](https://img.shields.io/badge/Canoas%2C%20RS%20%7C%20Brasil-009c3b?style=for-the-badge&logo=googlemaps&logoColor=white)](#)
[![Visitas](https://komarev.com/ghpvc/?username=charlesmrs&label=Visitas+ao+perfil&color=0A66C2&style=for-the-badge)](#)

</div>

---

## 🏭 Sobre mim

Sou Engenheiro de Produção com **19 anos na John Deere**, especializado em automação industrial, manutenção preditiva e gestão de projetos CAPEX. Hoje estou construindo a ponte entre o chão de fábrica e a inteligência artificial.

- 🔧 **Especialidade:** PLC Allen-Bradley · HMI · Redes Profibus/Profinet/Ethernet/IP
- 📊 **Gestão:** CAPEX · Facilities · Contratos de SLA · TPM
- 🤖 **Aprendendo agora:** Machine Learning & Visão Computacional — SCTEC/SC
- 🎓 **Também estudando:** Especialista em Inteligência Artificial — Alura + Santander Academy
- 🎯 **Objetivo:** Aplicar IA em ambientes industriais reais — detecção de falhas, manutenção preditiva inteligente e automação com visão computacional

> *"Na indústria chamamos de ruído quando um sensor envia dados corrompidos. No ML chamam de Garbage In, Garbage Out. Mesmo problema. Mesma consequência."*

---

## 📊 GitHub Stats

<div align="center">

<img height="160" src="https://github-readme-stats.vercel.app/api?username=charlesmrs&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true" />
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=charlesmrs&layout=compact&langs_count=6&theme=tokyonight&hide_border=true" />

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com/?user=DenverCoder1)](https://git.io/streak-stats)

</div>

---

## 🛠️ Stack & Ferramentas

**Automação Industrial**

![Allen-Bradley](https://img.shields.io/badge/PLC%20Allen--Bradley-FF6B35?style=flat-square&logoColor=white)
![HMI](https://img.shields.io/badge/HMI%20Programming-FF6B35?style=flat-square&logoColor=white)
![Profinet](https://img.shields.io/badge/Profinet%20%7C%20Profibus-FF6B35?style=flat-square&logoColor=white)
![Ethernet/IP](https://img.shields.io/badge/Ethernet%2FIP-FF6B35?style=flat-square&logoColor=white)

**Dados & Machine Learning**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?style=flat-square&logo=googlecolab&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-8B5CF6?style=flat-square&logoColor=white)
![Computer Vision](https://img.shields.io/badge/Computer%20Vision-8B5CF6?style=flat-square&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)

**Gestão & Sistemas**

![SAP](https://img.shields.io/badge/SAP%20MM%2FPDM-0FAAFF?style=flat-square&logo=sap&logoColor=white)
![PTC Windchill](https://img.shields.io/badge/PTC%20Windchill-0077C8?style=flat-square&logoColor=white)
![CAPEX](https://img.shields.io/badge/CAPEX%20Management-16A34A?style=flat-square&logoColor=white)
![TPM](https://img.shields.io/badge/TPM%20%7C%20Preditiva-16A34A?style=flat-square&logoColor=white)

---

## 🚀 Projetos em Destaque

### 🔬 [Pipeline ETL — Olist Data Sanitization](https://colab.research.google.com/github/charlesmrs/CodigoDosAlunos/blob/main/Mini_projeto.ipynb)
> Primeiro projeto do curso de ML — pipeline de sanitização de dados em **Python puro** (sem Pandas), processando datasets reais da Olist.

```
✅ 99.441 pedidos processados      ✅ 32.951 produtos analisados
✅ 610 categorias corrigidas       ✅ 619 pedidos cancelados identificados
✅ Hipótese de negócio validada    ✅ Relatório automatizado gerado
```

`Python` `CSV` `Regex` `ETL` `Data Cleaning` `Business Rules`

---

### 🔜 Próximos projetos

| Projeto | Descrição | Status |
|---|---|---|
| 🔍 Anomaly Detection Industrial | Modelo ML para detecção de falhas com dados de vibração/termografia | Em desenvolvimento |
| 👁️ Visual Inspection CV | Inspeção automática de peças com visão computacional | Planejado |
| 📊 Predictive Maintenance Dashboard | Dashboard com alertas preditivos integrado a dados de sensores | Planejado |

---

## 📚 Formação

| Curso | Instituição | Período |
|---|---|---|
| 🤖 Machine Learning & Visão Computacional | SCTEC — Santa Catarina | 2026 |
| 🧠 Especialista em Inteligência Artificial | Alura + Santander Academy | 2026 |
| 🎓 MBA em Gestão de Projetos | Universidade La Salle | 2023–2024 |
| ⚙️ Engenharia de Produção | Universidade La Salle | 2017–2023 |
| 🔌 Técnico em Eletrônica | ULBRA | 2009–2011 |

---

## 📈 Trajetória Profissional

```
2007 ──► Montador / Testes Elétricos (John Deere)
2013 ──► Técnico de Manutenção Eletrônica — PLC, CNC, Robôs Industriais
2018 ──► Especialista em Gestão de Mudanças — SAP, Windchill, ECR/ECO
2022 ──► Engineering Specialist — CAPEX, Facilities, TPM, ESG
2026 ──► Machine Learning & Computer Vision (em andamento)
```

---

## 🌱 O que estou construindo agora

- 📖 Cursando **Machine Learning e Visão Computacional** na SCTEC/SC
- 🔩 Desenvolvendo aplicações de **detecção de anomalias** com dados industriais reais
- 🏗️ Explorando **visão computacional aplicada a inspeção de qualidade** em linhas de montagem
- 💡 Unindo 19 anos de experiência industrial com IA para criar soluções que fazem sentido no chão de fábrica

---

<div align="center">

*"Dados limpos produzem modelos honestos — e modelos honestos produzem decisões confiáveis."*

⭐ Se algum projeto foi útil, considere deixar uma estrela!

</div>
